...
def parse_file(filename):
    try:
        ...
        if "DNS" in parser["Interface"]:
            config["WireGuard.Interface.DNS"] = " ".join(
                [s.strip() for s in parser["Interface"]["DNS"].split(",")]
            )

        if "PersistentKeepalive" in parser["Peer"]:
            config["WireGuard.Peer.PersistentKeepalive"] = parser["Peer"][
                "PersistentKeepalive"
            ]

        if "PresharedKey" in parser["Peer"]:
            config["WireGuard.Peer.PresharedKey"] = parser["Peer"]["PresharedKey"]
